...
test('P1 preferences keep sharing and notifications opt-in and normalize nested patches', () => {
  ...
  assert.equal(defaults.combinedUsageExpanded, false)
  ...
  assert.equal(prefs.merge(defaults, { combinedUsageExpanded: true }).combinedUsageExpanded, true)
  assert.equal(prefs.normalize({ combinedUsageExpanded: 'true' }).combinedUsageExpanded, false)
