...
function errorMessage(e) {
  ...
  if (e && typeof e === 'object') {
    if (e.name === 'AbortError') return 'Usage request timed out.'
...
async function readWithCreds(creds) {
...
async function read() {
  ...
  const initialAccessToken = creds.access_token || ''

  try {
    return await readWithCreds(creds)
  } catch (e) {
    const freshCreds = loadCreds()
    if (freshCreds?.access_token && freshCreds.access_token !== initialAccessToken) {
      try {
        return await readWithCreds(freshCreds)
      } catch (retryError) {
        e = retryError
      }
    }
...
module.exports = { read, _private: { loadCreds, readWithCreds, windowsFromData, parseQuota, errorMessage } }
